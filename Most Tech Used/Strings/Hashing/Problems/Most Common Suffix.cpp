// Codeforces : https://codeforces.com/gym/101502/problem/G
/*
...
struct PolyHash {
    vector<int64_t> suf; // Prefix and suffix hashes
    string s;
...
void solve() {
    ...
    cin >> n >> q;
    vector<string> h(n);
    vector<unordered_map<uint64_t,int>> mp(N);
    vector<int> freq(N);
    for (int i = 0; i < n; ++i) {
        cin >> h[i];
        string s = h[i];
        ...
        for (int j = s.size() - 1; j >= 0; --j) {
            mp[len][ph.rev_hash(j , s.size() - 1)]++;
            freq[len] = max(freq[len] , mp[len][ph.rev_hash(j , s.size() - 1)]);
            len++;
        }
    }
    while (q--) {
