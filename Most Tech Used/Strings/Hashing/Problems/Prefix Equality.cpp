// Atcoder : https://atcoder.jp/contests/abc250/tasks/abc250_e?lang=en
/*
...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        if (!mp.count(a[i]))
            mp[a[i]] = randomLongLong(1 , 1e17);
        if (!mp.count(b[i]))
            mp[b[i]] = randomLongLong(1 , 1e17);
    }
    for (int i = 0; i < n; ++i) {
        if(st1.count(a[i]))
            pre1[i + 1] = pre1[i];
        else
            pre1[i + 1] = pre1[i] ^ mp[a[i]];
        st1.insert(a[i]);
    }
    for (int i = 0; i < n; ++i) {
        if(st2.count(b[i]))
