// Atcoder : https://atcoder.jp/contests/abc367/tasks/abc367_f?lang=en
/*
...
void solve() {
    int n , q;
    cin >> n >> q;
    vector<int> a(n) , b(n) , pre1(n + 1 , 0) , pre2(n + 1 , 0);
    for (int i = 0; i < n; ++i) {
        cin >> a[i];
    }
    for (int i = 0; i < n; ++i) {
    ...
    for (int i = 0; i < n; ++i) {
        pre1[i + 1] = (pre1[i] + binpow(a[i])) % HashMod;
    }
    for (int i = 0; i < n; ++i) {
        pre2[i + 1] = (pre2[i] + binpow(b[i])) % HashMod;
    }
    while (q--) {
        int l , r , x , y;
        cin >> l >> r >> x >> y;
        int t1 = pre1[r] - pre1[l - 1];
        int t2 = pre2[y] - pre2[x - 1];
        t1 = (t1 + HashMod) % HashMod;
        t2 = (t2 + HashMod) % HashMod;
        if (t1 == t2)
            YES;
        else
            NO;
