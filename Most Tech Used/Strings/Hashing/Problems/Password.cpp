// Codeforces : https://codeforces.com/problemset/problem/126/B
/*
...
bool check(int x , PolyHash ph , int n) {
    for (int i = 1; i + x < n; i++) {
        if (ph.get_hash(i , i + x - 1) == ph.get_hash(0 , x - 1))
            return 1;
    }
    return 0;
}
...
void solve() {
    ...
    for (int i = 1; i < s.size(); ++i) {
        if(ph.get_hash(0 , i - 1) == ph.get_hash(s.size() - i , s.size() - 1))
            ans.push_back(i);
    }
    if (ans.empty()) 
        return void(cout << "Just a legend" << endl);
    int res = -1 , l = 0 , r = ans.size() - 1;
    while(l <= r) {
        int m = l + (r - l) / 2;
        if(check(ans[m] , ph , s.size())) {
            res = ans[m];
            l = m + 1;
        }
        else
